  ...
  <script>
    $(document).ready(function() {
      ...
      $(document).on('click', '.btnEdit', function() {
        ...
        $.ajax({
          method: 'GET',
          url: '<?php echo base_url('/admin_products/category_api?action=view_data'); ?>',
          data: {
            id: id
          },
      ...
      $('#editCategoryForm').submit(function(e) {
        ...
        $.ajax({
          method: 'POST',
          url: '<?php echo base_url('admin_products/category_api?action=edit_category'); ?>',
          data: data,
          success: function(res) {
            if (res.code == 201) {
      ...
      $('#deleteCategory').submit(function(e) {
        ...
        $.ajax({
          method: 'POST',
          url: '<?php echo base_url('admin_products/category_api?action=delete_category'); ?>',
          data: {
            id: id
          },
          success: function(res) {
      ...
      var table = $('#packageList').DataTable({
        "ajax": "<?php echo base_url('admin_products/category_api?action=list'); ?>",
        "columns": [{
            "data": "id"
          },
          {
          ...
          },
          {
            "mRender": function(data, type, row) {
              return '<div class="text-right"><a href="#" data-id="' + row.id + '" class="btn btn-warning btn-sm btnEdit"><i class="fa fa-edit"></i> Edit</a><a href="#" data-id="' + row.id + '" class="btn btn-danger btn-sm btnDelete"><i class="fa fa-trash"> Delete</i></a></div>';
            }
          }
        ],
        "language": {
      ...
      $('#addCategoryForm').submit(function(e) {
        ...
        $.ajax({
          method: 'POST',
          url: '<?php echo base_url('admin_products/category_api?action=add_category'); ?>',
          data: data,
          context: this,
          success: function(res) {
            if (res.data) {
